...
{
    Task<(string AccessToken, string RefreshToken, DateTime ExpiresAt)> AuthenticateAsync(string email, string password, string ipAddress, string userAgent, CancellationToken ct = default);
    ...
    Task<bool> SetupMfaAsync(Guid userId, CancellationToken ct = default);
